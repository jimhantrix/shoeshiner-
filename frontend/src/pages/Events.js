...
class EventsPage extends Component {
  ...
  modalConfirmHandler = () =>{
    ...
    .then(resData => {
      this.setState( prevSate =>{
        ...
        const updatedEvents = [ ...prevSate.events];
        updatedEvents.push({
  ...
  showDetailHandler = eventId => {
    this.setState(prevSate => {
      const selectedEvent = prevSate.events.find(e => e._id === eventId);
...

        {
            this.state.selectedEvent && (
              <Modal
                ...
                canConfirm
                onCancel={this.modalCancelHandler}
                onConfrim ={this.bookEventHandler}
                confirmText = {this.context.token ? 'Book': 'Confirm'}>

                <h1>{this.state.selectedEvent.title}</h1>
                <h2>${this.state.selectedEvent.price} - {' '}
                {new Date (this.state.selectedEvent.date).toLocaleDateString()}</h2>
